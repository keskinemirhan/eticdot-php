...
if (isset($_REQUEST["search"])) {
    ...
    where c.id = p.categoryId and v.id = p.vendorId and (c.name like '%$queryStr%' or
    p.name like '%$queryStr%') and v.id = '$vendorId' limit $limit offset $offset  ");
    ...
    where c.id = p.categoryId and v.id = p.vendorId and (c.name like '%$queryStr%' 
     or
    p.name like '%$queryStr%') and v.id = '$vendorId' limit $limit offset $offset  ")->fetch_row()[0];
    ...
    p.id,p.image, p.name, v.name, c.name ,p.price, p.prevPrice   
    from category as c, product as p, vendor as v
    where c.id = p.categoryId and v.id = p.vendorId and v.id = '$vendorId' ");
    $product_count = $mysqli->query("SELECT count(*) from product where vendorId = '$vendorId' ")->fetch_row()[0];
    $total_page = ceil($product_count / $limit);
}
...
<div class="product-list-container">
    ...
    <hr>
    <div class="m-2">
        <form class="d-flex justify-content-between" method="get" action="vendor-product-list.php">
            <div class="d-flex align-items-center">
                ...
                <a href="vendor-product-add.php" class="btn btn-success mx-2">Add Product</a>
            </div>
            <div class="d-flex align-items-center">
        ...
        <tbody>
            <?php while ($product = $products_result->fetch_row()) { ?>
                <tr>
                    ...
                    <td>
                        <form action="vendor-product-list.php">
                            <a class="btn btn-primary" href="<?php echo "vendor-product-update.php?id=$product[0]" ?>">Update</a>
                            <a class="btn btn-danger" href="<?php echo "vendor-product-update.php?id=$product[0]" ?>">Delete</a>
